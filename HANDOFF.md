...
- 콘텐츠: MCQ 22개(python 12 / java 10), 플래시카드 20개(cards/patterns.json),
...
task-dependency-check, LRU Cache → lru-session-cache, Trapping Rain Water I
→ profile-rainwater. **중복 주의**: 같은 문제가 다시 들어오면 새 파일을
만들지 말고 기존 md와 대조 후 필요한 부분만 갱신.

## 카테고리 (problems/ 폴더)

arrays-intervals, binary-search, design, dynamic-programming, graphs-trees,
heaps-stacks, strings-hashing, two-pointers-sliding-window.
필요시 추가 후보: implementation-simulation, math-bits.
